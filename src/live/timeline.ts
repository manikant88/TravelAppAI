...
export function dayStops(day: LiveDay): LiveDayStop[] {
  ...
  return [...breakfast, ...activities.slice(0, 1), ...lunch, ...activities.slice(1), ...dinner];
}
...
export function projectLiveDay(day: LiveDay, startMinutes: number | null = 600) {
  ...
  for (let i = 0; i < stops.length; i++) {
    ...
    const start: number | null = ready !== null ? Math.max(ready, visit.kind === 'meal' ? visit.targetStartMinutes : ready) : null;
    const waitMinutes = ready !== null && start !== null ? start - ready : null;
    cursor = start !== null ? start + visit.durationMinutes : null;
    rows.push({ visit, leg, departure, arrival, start, end: cursor, bufferMinutes, waitMinutes });
  }
