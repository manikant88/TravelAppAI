...
function explicitTravellers(message: string): TripRequest["travellers"] | undefined {
  ...
  const totalMatch = message.match(/\b(?:for\s+)?(\d|one|two|three|four|five|six)\s+(?:people|persons|travellers|travelers|guests)\b/i);
  const adultMatch = message.match(/\b(\d|one|two|three|four|five|six)\s+adults?\b/i);
  ...
  if (!totalMatch && adultMatch) {
    const childMatch = message.match(/\b(\d|one|two|three|four|five|six)\s+(?:children|kids|child)\b/i);
    const seniorMatch = message.match(/\b(\d|one|two|three|four|five|six)\s+seniors?\b/i);
    const children = childMatch ? countValue(childMatch[1]!) : 0;
    const seniors = seniorMatch ? countValue(seniorMatch[1]!) : 0;
    if (total + children + seniors > 6) return undefined;
    return [
  ...
  if (total < 1 || total > 6) return undefined;
  const childCountMatch = message.match(/\b(\d)\s+(?:children|kids)\b/i);
...
function explicitDateRange(message: string, today: string): { startDate: string; endDate: string } | undefined {
  const values = [...message.matchAll(/\b(?:20\d{2}-\d{2}-\d{2}|\d{1,2}(?:st|nd|rd|th)?[\s-]+(?:january|February|march|april|may|june|july|august|september|october|november|december)(?:[\s,]+20\d{2})?)\b/gi)].map((match) => parseExplicitDate(match[0]!)).filter((value): value is string => Boolean(value));
  if (values.length >= 2) return { startDate: values[0]!, endDate: values[1]! };
  const start = values[0] ?? relativeWeekend(message, today)?.startDate;
  const nights = explicitNightCount(message);
  if (!start || !nights) return undefined;
  return { startDate: start, endDate: addCalendarDays(start, nights) };
}
...
function explicitLocationQuery(message: string, kind: "origin" | "destination"): string | undefined {
  const pattern = kind === "origin"
    ? /\b(?:from|leaving from|departing from|starting from)\s+([a-z][a-z\s-]*?)(?=\s+(?:to|for|on|starting|between|under|with|and)\b|[,.]|$)/i
    : /\b(?:to|in|visit(?:ing)?)\s+([a-z][a-z\s-]*?)(?=\s+(?:from|for|on|starting|between|under|with|and)\b|[,.]|$)/i;
...
async function deterministicIntent(
  ...
  repository: ReturnType<typeof createInventoryRepository>,
  today: string,
): Promise<NaturalTripIntent | undefined> {
  const originQuery = explicitLocationQuery(message, "origin");
  const destinationQuery = explicitLocationQuery(message, "destination");
  const asksForRecommendations = /\b(?:recommend|suggest|help me choose|open to|anywhere|somewhere)\b/i.test(message);
  ...
  const pace = /\b(relaxed|balanced|packed)\b/i.exec(message)?.[1]?.toLocaleLowerCase("en") as NaturalTripIntent["pace"] ?? null;
  const supportedThemes = new Set(catalog.supportedThemes.map((theme) => theme.toLocaleLowerCase("en")));
  const interests = [...supportedThemes].filter((theme) => new RegExp(`\\b${theme.replace(/[.*+?^${}()|[\\]\\\\]/g, "\\\\$&")}\\b`, "i").test(message));
  ...
  const destinationIntent = asksForRecommendations
    ? { kind: "open" as const }
  ...
  if (!origin || !destinationIntent || !startDate || !endDate || resolvedTravellers.length === 0) return undefined;
  if (originQuery && !origin.id) return undefined;
  if (destinationQuery && !destination) return undefined;
  return {
...
function relativeWeekend(
  message: string,
  today: string,
): { startDate: string; endDate: string } | undefined {
  if (!/\b(?:this|upcoming)\s+weekend\b/i.test(message)) return undefined;
...
function deterministicMessage(
  ...
  return "I converted your message into a grounded trip brief. Review the details, then build the trip when you’re ready.";
}
...
export async function runNaturalIntake(
  ...
  let deterministic: NaturalTripIntent | undefined;
  try {
  ...
  if (deterministic) {
    ...
    try {
      intent = naturalTripIntentSchema.parse(
      ...
      throw new NaturalIntakeError(
        "MODEL_FAILURE",
        "The travel planner could not interpret that trip brief",
        502,
        true,
      );
    }
  ...
  const relativeDates = relativeWeekend(parsed.data.message, today);
