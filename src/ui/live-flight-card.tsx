...
export function LiveFlightCard({ offer, direction, from, to, travellers, selected = true, locked = false, onLock, onChange, onSelect, selectionBusy = false, decisionNote }: { offer: TransportOffer; direction: 'outbound' | 'return'; from: LivePlace; to: LivePlace; travellers: number; selected?: boolean; locked?: boolean; onLock?(): void; onChange?(): void; onSelect?(): void; selectionBusy?: boolean; decisionNote?: string }) {
  const segment = offer.segments[0];
  return <article className={`itinerary-card itinerary-flight-card live-route-card ${selected ? 'is-suggested' : ''}`}>
    <header className="itinerary-card-header"><span className="card-kind-icon" aria-hidden="true"><AppIcon name="flight" /></span><strong>{selected ? 'Selected' : 'Alternative'} {direction} flight · {segment.number} · {duration(offer.durationMinutes)}</strong><div className="live-card-actions">{onLock && <Button variant="text" size="sm" aria-pressed={locked} disabled={selectionBusy} onClick={onLock}>{locked ? 'Unlock' : 'Lock'}</Button>}{onLock && (onChange || onSelect) && <span aria-hidden="true">·</span>}{onChange && <Button variant="text" size="sm" onClick={onChange} disabled={selectionBusy || locked}>Change</Button>}{onSelect && <Button size="sm" onClick={onSelect} disabled={selectionBusy}>Select flight</Button>}</div></header>
