...
import type { LiveDay, LivePlan, LivePlace, LiveSelectionRequest, LiveTravelOption } from '@/live/contracts';
...
export function LiveOptionDrawer({ plan, picker, busy, onSelect, onClose }: {
  ...
  const title = picker.kind === 'hotel' ? 'Change stay' : picker.kind === 'activity' ? 'Change activity' : picker.kind === 'flight' ? `Change ${picker.direction} flight` : `Change ${picker.direction} travel`;
  const noun = picker.kind === 'hotel' ? 'stays' : picker.kind === 'activity' ? 'activities' : picker.kind === 'flight' ? 'flights' : 'routes';
...
function OptionRow({ option, picker, plan, currentHotel, currentVisit, busy, onSelect }: { option: Option; picker: LiveOptionPicker; plan: LivePlan; currentHotel?: LivePlace; currentVisit?: LiveDay['visits'][number]; busy: boolean; onSelect(): void }) {
  const note = impact(option, picker, plan, currentHotel, currentVisit?.place);
...
function impact(option: Option, picker: LiveOptionPicker, plan: LivePlan, currentHotel?: LivePlace, currentVisit?: LivePlace) {
