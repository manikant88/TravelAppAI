...
export function LivePlaceCard({ place, kind, heading, subtitle, selected = false, estimate = '', onEstimate, locked = false, onLock, onChange, onSelect, selectionBusy = false, decisionNote }: {
  ...
  return <PlaceCardFrame kind={kind} heading={heading} actions={actions} className={`live-place-card ${selected ? 'is-suggested' : ''}`}>
    <div className={kind === 'hotel' ? 'hotel-card-body' : 'activity-card-body'}>
      ...
      <div className="hotel-details">
        {(place.rating !== undefined || stayOffer?.propertyFacts.starRating !== undefined) && <div className="hotel-rating">
          ...
          <span><AppIcon name="star" size={13} /> {place.rating !== undefined ? `${place.source} rating` : 'Hotel classification'}</span>
        ...
        <div className="card-price">
          ...
          <span>{stayOffer?.totalPrice
            ? `${stayOffer.source.evidenceKind === 'sandbox' ? 'Sandbox' : 'Supplier'} total for ${stayOffer.rooms} room${stayOffer.rooms === 1 ? '' : 's'} · ${stayNights} night${stayNights === 1 ? '' : 's'} · ${money(stayOffer.price.amount)} average per room/night`
    ...
    <footer className="live-card-footer">
      {(place.editorialSummary || place.amenities?.length) && <section className="live-place-about" aria-label={`About ${place.name}`}>
        ...
        {!!place.amenities?.length && <ul>{place.amenities.map(amenity => <li key={amenity}><AppIcon name="check" size={13} /> {amenity}</li>)}</ul>}
      </section>}
      <div className="live-card-meta-row">
        <div className="live-card-source-note">
          <span>{place.source} · {stayOffer ? `${stayOffer.source.evidenceKind === 'sandbox' ? 'Sandbox offer' : 'Dated offer'} checked ${new Date(stayOffer.source.checkedAt!).toLocaleString()}` : kind === 'hotel' ? 'Stay details are provisional' : kind === 'meal' ? 'Restaurant and dietary details require confirmation' : 'Visit duration is an estimate'}; verify important details.</span>
