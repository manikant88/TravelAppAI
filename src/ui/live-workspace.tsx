...
import { emptyLiveBrief, type LivePlan, type LiveRequest, type LiveDay, type LiveTravelOption, type LiveSelectionRequest, type LiveSelectionResponse } from '@/live/contracts';
...
import { LiveTripBriefBar, LiveTripEssentials, type BriefFact } from './live-trip-essentials';
  ...
  return <div className="live-timeline day-events">
    ...
    {rows.map(({ visit, leg, departure, arrival, start, end, bufferMinutes: connectionBuffer, waitMinutes }, index) => <div key={`${visit.kind}-${visit.place.id}-${index}`}>
      ...
      <div className="live-timeline-event" data-focus={`place:${visit.place.id}`}>
        ...
        {visit.kind === 'meal'
          ? <LivePlaceCard place={visit.place} kind="meal" heading={`${visit.type[0].toUpperCase()}${visit.type.slice(1)}`} subtitle={`${mealRouteNote(visit.routeFit)} · ${visit.dietaryNote}`} secondaryDetails={[visit.hoursNote ?? 'Opening hours unavailable', visit.shiftedByMinutes ? `Shifted ${Math.abs(visit.shiftedByMinutes)} min ${visit.shiftedByMinutes > 0 ? 'later' : 'earlier'} within the flexible meal window` : 'Placed within the preferred meal window']} estimate={estimates[visit.place.id] ?? ''} onEstimate={value => onEstimate(visit.place.id, value)} locked={lockedMealKeys.includes(`${dayIndex}:${visit.type}`)} selectionBusy={busy} onLock={() => onLockMeal(dayIndex, visit.type, !lockedMealKeys.includes(`${dayIndex}:${visit.type}`))} onChange={() => onChangeMeal(dayIndex, visit.type)} />
          : <LivePlaceCard place={visit.place} kind="activity" heading={`Stop ${visit.sourceIndex + 1} · ${activityTimingLabel(visit)}`} subtitle={`${visit.hoursNote ?? 'Opening hours unavailable'}${visit.mealCoverage ? ` · Includes ${visit.mealCoverage.type}` : ''}`} secondaryDetails={[visit.durationProfile ? `${visit.durationProfile.kind.replace('_', ' ')} duration · ${visit.durationProfile.minimumMinutes}–${visit.durationProfile.maximumMinutes} min` : 'Estimated visit length', ...(visit.durationProfile?.difficulty ? [`${visit.durationProfile.difficulty} difficulty`] : []), ...(visit.durationProfile?.daylightWindow ? ['Daylight-sensitive'] : []), ...(visit.mealCoverage ? [`${visit.mealCoverage.sourceLabel}: ${visit.mealCoverage.note}`] : [])]} estimate={estimates[visit.place.id] ?? ''} onEstimate={value => onEstimate(visit.place.id, value)} locked={lockedActivityIds.includes(visit.place.id)} selectionBusy={busy} onLock={() => onLockActivity(visit.place.id, !lockedActivityIds.includes(visit.place.id))} onChange={() => onChangeActivity(dayIndex, visit.sourceIndex)} />}
      </div>
    </div>)}
    {rows.length > 0 && <>
...
export default function LiveWorkspace({ initialPrompt = '', autoSubmitInitialPrompt = false }: { initialPrompt?: string; autoSubmitInitialPrompt?: boolean }) {
  ...
  const sendMessage = useCallback(async (rawMessage: string) => {
    ...
    const message = rawMessage.trim(); const previous = messages;
    ...
    try {
      ...
      if (result.plan) { setPlan(result.plan); setPlanNeedsRefresh(false); setDay(0); setEstimates({}); setLastImpact(undefined); }
  ...
  function editBrief(field: LiveEssentialField) {
    const fact = field === 'travellers' ? 'guests' : field === 'origin' || field === 'destination' || field === 'dates' ? field : field === 'transport' || field === 'dining' || field === 'pace' ? 'preferences' : undefined;
  ...
  return <main className="live-shell">
    ...
    <div className="live-layout">
      <section className="live-chat planner-panel" aria-label="Planning conversation">
        ...
        {chatSuggestions.length ? <div className="conversation-actions" aria-label="Suggested answers">{chatSuggestions.map(suggestion => <Chip className="guided-action-chip" key={suggestion.label} onClick={() => void sendMessage(suggestion.message)}>{suggestion.label}</Chip>)}</div> : null}
      ...
      <section className="live-results" ref={results} aria-label={plan ? 'Live itinerary' : 'Trip setup'}>
        ...
        {!plan && !busy && briefInterpreted ? <LiveTripEssentials brief={brief} busy={busy} onEdit={editBrief} onSubmit={message => void sendMessage(message)} /> : null}
        {!plan && !busy && !briefInterpreted ? <div className="live-empty"><span className="eyebrow">{messages.length ? 'TRY AGAIN' : 'YOUR NEXT ADVENTURE'}</span><h2>{messages.length ? 'Your request still needs a successful read.' : <>Start with an idea.<br />Make it a trip.</>}</h2><p>{messages.length ? 'Edit or resend your message in chat. The Trip Brief will appear only after the planner validates what it understood.' : 'Share a destination, dates, and who is coming. The planner will interpret your message before showing any Trip Brief fields.'}</p></div> : null}
        {plan && <>
          <header className="live-trip-heading"><div><p className="eyebrow">YOUR TRIP</p><h2>{plan.brief.destination}</h2><p>From {plan.brief.origin} · {dateLabel(plan.brief.startDate!)} – {dateLabel(plan.days.at(-1)!.date)} · {plan.brief.travellers} travellers · {plan.brief.days! - 1} nights · {plan.scheduling?.pace ?? 'balanced'} pace{plan.scheduling?.paceDefaulted ? ' (default)' : ''}</p></div><Badge tone="warning">{planNeedsRefresh ? 'Previous plan' : 'Provisional plan'}</Badge></header>
